...
class SortByPatternTest < Minitest::Test
  ...
  def test_sort_alphabetically
    ...
    transformed = []
    words.each do |word|
      transformed << [word.downcase, word] # in order to be alphabetized, letters have to be the same case, hence String#downcase call
    end
  ...
  def test_sort_alphabetically_by_last_letter
    ...
    things.each do |thing|
      transformed << [thing[-1].upcase, thing]
    end
    transformed = transformed.sort
    sorted = []
  ...
  def test_sort_by_distance
    ...
    distances.each do |distance|
      transformed << [distance.split("cm").join.to_i, distance]
    end
    transformed = transformed.sort
    sorted = []
  ...
  def test_sort_by_proximity_to_ten
    ...
    prices.each do |price|
      transformed << [(price - 10.00).abs, price]
    end
    transformed = transformed.sort
    sorted = []
  ...
  def test_sort_by_number_of_cents
    ...
    transformed = []
    prices.each do |price|
      new_price = price.to_s.split(".")
      transformed << [new_price[1], price]
    end
    transformed = transformed.sort
    sorted = []
    transformed.each do |sort_key, price|
